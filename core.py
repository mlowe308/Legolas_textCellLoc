...
# still used rpyc.classic.connect in def connect_pi1 and connect_pi2
# 6/19/24 Around line 660, in def pH_measure, read voltage instead of pH. 
# 6/28/24 def acquire (line 474) changed 'full_down' to 'intake', then changed back on 7/1/24
# 7/1/24 def deposition (line 547 and 559) added "full_down" and "full_up" commands
...
def load_from_config(config_path):
    ...
    #17
    print("gets to #17. finishes connect_pi1")
    ...
    #18
    print("gets to #18. Finishes connect_pi2")
    ...
    #10
    print("gets to #10 in load_from_config")
    ...
    # 5
    print("gets to #5 in load_from_config")
...
def save_cell_map(cell_map, path):
    ...
    print("cell map", cell_map)
    cell_map_text_path = path / "cell_map.txt"
    with cell_map_text_path.open("w") as f:
        shape = cell_map.shape
        f.write( "%d %d %d\n" % (shape[0], shape[1], shape[2]) )
        for ix in range(shape[0]):
            for iy in range(shape[1]):
                f.write( "%.6f %.6f\n" % (cell_map[ix][iy][0], cell_map[ix][iy][1]) )
    return str(cell_map_text_path)
...
def load_cell_map(cell_map_path):
    ...
    with Path(cell_map_path).open("r") as f:
        shape = tuple([int(v) for v in f.readline().split()])
        cell_map = np.zeros( shape )
        for ix in range(shape[0]):
            for iy in range(shape[1]):
                xy = tuple([float(v) for v in f.readline().split()])
                cell_map[ix][iy][0] = xy[0]
                cell_map[ix][iy][1] = xy[1]
    return cell_map
...
class DepositionDevice(DeviceOnStage):
    ...
    def deposition(self, vol=None, dep_degree=None, location=None, row=None, col=None, x_degree=None, y_degree=None):
        ...
        self.move_to(location=location, row=row, col=col, x_degree=x_degree, y_degree=y_degree)
        self.to_zpos("full_down")             #Miller and Adi added
        # if location is not None:
        #     self.move_to_loc(location)
        # elif row is not None and col is not None:
        #     self.move_to_cell(row, col)
        ...
        self.volume -= vol
        self.to_zpos("full_up")      #Miller and Adi added this
...
class pHDevice(DeviceOnStage):
    ...
    def report_pH(self, stable_time=15):
        ...
        for i in range(stable_time):
            ...
            try:
                ...
                print(f"fail to decode message from pH device. raw message: {x}.")                
            time.sleep(1)
    ...
    def pH_measure(self, stable_time, location=None, row=None, col=None, x_degree=None, y_degree=None):                               
        ...
        shake = 30
        voltage = self.report_pH(stable_time)        #added
        ##self.to_zpos("full_up")
        ...
        #for i in range (3):                         # for shaking off droplets
            ...
            #self.motor_pH.run_for_degrees(-shake)
        if self.verbose:
            print(f'voltage for pH is Measured as: {voltage:.2f}', )     #added Read voltage instead.
            ##print(f'pH is Measured as: {pH_value:.2f}', )
        return voltage                               # added
        ##return pH_value
